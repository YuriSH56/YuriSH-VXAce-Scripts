...
class Window_MusicControl < Window_Base
  ...
  def initialize
    ...
    @track_id = -1
    @track_data = nil
    @volume = YuriSH::MusicPlayer::VOLUME_DEFAULT
    @pitch = YuriSH::MusicPlayer::PITCH_DEFAULT
    @last_pos = $MKXP ? 0.0 : 0
  ...
  #--------------------------------------------------------------------------
  def get_sample_rate
    return YuriSH::MusicPlayer::DEFAULT_RATE if @track_data.nil?
    @track_data.include?(:sample_rate) ? @track_data[:sample_rate] : YuriSH::MusicPlayer::DEFAULT_RATE
  end
  #--------------------------------------------------------------------------
  # * Set Track
  #--------------------------------------------------------------------------
  def set_track(id)
    if id != @track_id
      stop
      @track_id = id
      @track_data = YuriSH::MusicPlayer::TRACKS[@track_id]
      @last_pos = $MKXP ? 0.0 : 0
      @start_time = $MKXP ? 0.0 : 0
      play
    end
  ...
  def move_by(seconds)
    ...
    if $MKXP
      ...
      @last_pos += YuriSH::MusicPlayer::VXACE_HZ_RATE * seconds
  ...
  def change_pitch(value)
    p_min = YuriSH::MusicPlayer::PITCH_RANGE.min
    p_max = YuriSH::MusicPlayer::PITCH_RANGE.max
  ...
  def get_duration_params
    ...
    if $MKXP
      ...
      cur_dur += (@last_pos / YuriSH::MusicPlayer::VXACE_HZ_RATE.to_f)
    end
  ...
  def draw_progress_gauge(rate)
    draw_gauge(0, line_height - 12, contents_width, rate,
      text_color(YuriSH::MusicPlayer::MAIN_GAUGE[0]),
      text_color(YuriSH::MusicPlayer::MAIN_GAUGE[1]))
  end
  #--------------------------------------------------------------------------
  # * Draw Volume Control
  #--------------------------------------------------------------------------
  def draw_volume_control
    ...
    next_pos += 66
    draw_icon(YuriSH::MusicPlayer::MINUS_ICON, next_pos, d_height)
    
    next_pos += 30
    draw_gauge(next_pos, d_height, 100, @volume / 100.0,
      text_color(YuriSH::MusicPlayer::VOLUME_GAUGE[0]),
      text_color(YuriSH::MusicPlayer::VOLUME_GAUGE[1]))
      
    gauge_rect = Rect.new(next_pos, d_height, 100, line_height)
    draw_text(gauge_rect, "%d%%" % @volume, 1)
    
    next_pos += 106
    draw_icon(YuriSH::MusicPlayer::PLUS_ICON, next_pos, d_height)
  end
  #--------------------------------------------------------------------------
  # * Draw Pitch Control
  #--------------------------------------------------------------------------
  def draw_pitch_control
    ...
    draw_gauge(next_pos, d_height, 100, (@pitch - 50) / 100.0,
      text_color(YuriSH::MusicPlayer::PITCH_GAUGE[0]),
      text_color(YuriSH::MusicPlayer::PITCH_GAUGE[1]))
  ...
  def handle_pitch
    if Input.trigger?(:LEFT) or Input.repeat?(:LEFT)
      change_pitch(@pitch - YuriSH::MusicPlayer::PITCH_STEP)
      Sound.play_cursor
  ...
  def handle_volume
    if Input.trigger?(:LEFT) or Input.repeat?(:LEFT)
      change_volume(@volume - YuriSH::MusicPlayer::VOLUME_STEP)
      Sound.play_cursor
    elsif Input.trigger?(:RIGHT) or Input.repeat?(:RIGHT)
      change_volume(@volume + YuriSH::MusicPlayer::VOLUME_STEP)
      Sound.play_cursor
  ...
  def handle_move
    if @playing
      i_step = YuriSH::MusicPlayer::SEEK_STEP
      if Input.trigger?(:LEFT) or Input.repeat?(:LEFT)
...
class Window_MusicSelect < Window_Selectable
  ...
  #--------------------------------------------------------------------------
  def get_data(index)
    i_key = YuriSH::MusicPlayer::TRACKS.keys[index]
    i_item = YuriSH::MusicPlayer::TRACKS[i_key]
  ...
  #--------------------------------------------------------------------------
  def item_max
    return YuriSH::MusicPlayer::TRACKS.size
  end
  #--------------------------------------------------------------------------
  # * Select Item
  #--------------------------------------------------------------------------
  ...
  def item_unlocked?(index)
    return true unless YuriSH::MusicPlayer::UNLOCK_MODE
    data = get_data(index)
  ...
  def draw_item(index)
    ...
    change_color(text_color(YuriSH::MusicPlayer::ID_COLOR), is_enabled)
    ...
    draw_text(text_rect,
      is_unlocked ? data[:data][:name] : YuriSH::MusicPlayer::LOCKED_NAME,
      0)
    
    if self.current_track == data[:key]
      if self.is_playing
        draw_icon(YuriSH::MusicPlayer::PAUSE_ICON,
          icon_x, icon_y, is_enabled)
      else
        draw_icon(YuriSH::MusicPlayer::PLAY_ICON,
          icon_x, icon_y, is_enabled)
      end
...
if YuriSH::MusicPlayer::UNLOCK_MODE
...
class Game_MusicPlayer
  ...
  #--------------------------------------------------------------------------
  def unlocked?(id)
    @data.include?(id) or YuriSH::MusicPlayer::TRACKS[id].fetch(:unlocked, false)
  end
  #--------------------------------------------------------------------------
  # * Unlock Music Tracks
  #--------------------------------------------------------------------------
  ...
  #--------------------------------------------------------------------------
  def unlock_all
    @data = YuriSH::MusicPlayer::TRACKS.keys
  end
  #--------------------------------------------------------------------------
  # * Lock All Tracks
  #--------------------------------------------------------------------------
...
module DataManager
  ...
  def self.load_music_data
    if music_data_exists?
      $game_music_player = load_data(YuriSH::MusicPlayer::DATA_PATH)
    else
  ...
  def self.save_music_data_without_rescue
    File.open(YuriSH::MusicPlayer::DATA_PATH, "wb") do |file|
  ...
  def self.delete_music_data
    File.delete(YuriSH::MusicPlayer::DATA_PATH) rescue nil
  end
  #--------------------------------------------------------------------------
  # * Returns True If Music Data Exists.
  #--------------------------------------------------------------------------
  def self.music_data_exists?
    FileTest.exist?(YuriSH::MusicPlayer::DATA_PATH)
  end
end
