...
module ParallaxManager
  ...
  @dirty = {}
  #--------------------------------------------------------------------------
  # * Adds Layer To Parallax
  #--------------------------------------------------------------------------
  def self.add_layer(graphic, id, data)
    return if graphic.nil?
    @parallaxes[graphic] = {} unless @parallaxes.has_key?(graphic)
    @parallaxes[graphic][id] = data
    set_dirty(graphic, true)
  end
  #--------------------------------------------------------------------------
  # * Gets Layers Associated With Parallax Graphic
  #--------------------------------------------------------------------------
  def self.get_layers(graphic)
    return @parallaxes[graphic] if @parallaxes.has_key?(graphic)
    return nil
  end
  #--------------------------------------------------------------------------
...
class Spriteset_Map
  ...
  def create_layers
    @layers = {}
    layer_data = ParallaxManager.get_layers(@parallax_name)
  ...
  #--------------------------------------------------------------------------
  def update_layers
    if ParallaxManager.dirty?(@parallax_name)
      ParallaxManager.set_dirty(@parallax_name, false)
      dispose_layers
      create_layers
  ...
  def update_parallax
    if @parallax_name != $game_map.parallax_name
      ParallaxManager.set_dirty($game_map.parallax_name, true)
    end
